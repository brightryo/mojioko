...
function FontLicenseEntry({ meta }: { meta: FontMeta }) {
  ...
  return (
    <div className="rounded-md border border-line bg-surface-1 px-3 py-2 space-y-1.5">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-2 min-w-0">
          <span
            className="text-body font-medium text-fg-primary truncate"
        ...
        <div className="flex items-center gap-1 shrink-0">
          <button
            type="button"
            onClick={loadOfl}
            className="text-caption text-fg-secondary hover:text-fg-primary transition-colors"
          >
          ...
          <a
            ...
            className="inline-flex items-center gap-1 text-caption text-fg-secondary hover:text-fg-primary transition-colors"
            onClick={(e) => {
              e.preventDefault()
      ...
      <div className="text-body-sm text-fg-secondary">
        <span className="font-medium text-fg-primary/80">{t('fontLicenses.copyrightHeader')}:</span>{' '}
        {meta.copyright}
      </div>
      {oflLoaded && oflText && (
        <pre className="mt-2 max-h-[200px] overflow-y-auto text-caption font-mono text-fg-secondary/80 bg-surface-2/30 rounded p-2 whitespace-pre-wrap">
          {oflText}
        </pre>
      )}
    </div>

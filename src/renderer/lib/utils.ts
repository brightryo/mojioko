 ...
 * size token (e.g. `text-body`) and a colour token (e.g. `text-zinc-950`)
 ...
 * `text-zinc-950` because cva concatenated `variant.primary` BEFORE
 * `size.md`, so `text-body` overrode `text-zinc-950` and the rendered
 ...
 * correct classification: `text-body` is a font-size, `text-zinc-950` is

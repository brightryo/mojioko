...
const COMMANDS: CommandDoc[] = [
  ...
  },
  {
    ...
    optionSpecs: [
      ...
      // REQ-0531 §2-1 — the axis is stated because it is the one thing about
      // this flag a caller cannot infer, and getting it wrong returns a
      // plausible-looking image of the wrong moment.
      { flag: '--time', type: 'string', required: true, desc: '抽出する時刻（秒。トリミング後＝焼き上がり動画上の位置）。カンマ区切りで複数可（最大20・例 1.0,3.5,7.2）' },
  ...
  {
    name: 'add_cue',
    summary: 'cue を新規追加（挿入位置・既定スタイルは GUI の「行を追加」と同じ）',
    usage: 'mojioko add_cue <in.mojioko> -o <out> --start 1.0 --end 3.0 [--text "..."]',
    positionals: [{ name: 'subtitle', required: true, desc: '.mojioko' }],
    optionSpecs: [
      OUT_REQ,
      { flag: '--start', type: 'string', required: true, desc: '開始秒' },
      { flag: '--end', type: 'string', required: true, desc: '終了秒（start より後）' },
      { flag: '--text', type: 'string', required: false, desc: '本文（既定は空）' },
      OVERWRITE,
    ],
    examples: [
      'mojioko add_cue p.mojioko -o p.mojioko --start 12.5 --end 15.0 --text "ここに字幕"',
    ],
    errorCodes: ['INPUT_NOT_FOUND', 'UNSUPPORTED_FORMAT', 'USAGE', 'OUTPUT_WRITE_FAILED'],
  },
  {
    name: 'duplicate_cue',
    summary: 'cue を複製（スタイル・単語タイミング込み・直後に挿入）',
    usage: 'mojioko duplicate_cue <in.mojioko> -o <out> --index 0',
    positionals: [{ name: 'subtitle', required: true, desc: '.mojioko' }],
    optionSpecs: [
      OUT_REQ,
      { flag: '--index', type: 'string', required: false, desc: '対象 cue の番号（--id と排他）' },
      { flag: '--id', type: 'string', required: false, desc: '対象 cue の id（--index と排他）' },
      OVERWRITE,
    ],
    examples: ['mojioko duplicate_cue p.mojioko -o p.mojioko --index 3'],
    errorCodes: ['INPUT_NOT_FOUND', 'UNSUPPORTED_FORMAT', 'USAGE', 'OUTPUT_WRITE_FAILED'],
  },
  {
    name: 'reset_cue',
    summary: 'cue を文字起こし直後の状態へ戻す（GUI の「行のリセット」と同じ）',
    usage: 'mojioko reset_cue <in.mojioko> -o <out> --index 0',
    positionals: [{ name: 'subtitle', required: true, desc: '.mojioko' }],
    optionSpecs: [
      OUT_REQ,
      { flag: '--index', type: 'string', required: false, desc: '対象 cue の番号（--id と排他）' },
    ...
    examples: ['mojioko reset_cue p.mojioko -o p.mojioko --index 3'],
    errorCodes: ['INPUT_NOT_FOUND', 'UNSUPPORTED_FORMAT', 'USAGE', 'OUTPUT_WRITE_FAILED'],
  },
  {
    name: 'convert',

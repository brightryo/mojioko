...
export function registerTranscriptionHandlers(): void {
  ipcMain.handle(
    Channels.transcriptionCheckModel,
    (_event, modelId: string): OkResult<{ installed: boolean; sizeMB: number }> | ErrResult => {
      try {
  ...
  ipcMain.handle(
    ...
    async (
    ...
    ): Promise<OkResult<{ channelId: string }> | ErrResult> => {
      ...
      transcribe(fullRequest, (evt) => {
        if (evt.event === 'failed') {
          log.error(
            `[ipc/transcription] failed (whisper): model=${request.modelId} reason=${evt.error}`
          )
        }
        ...
        .then(async () => {
          ...
          if (needsPreviewMix) {
            ...
            try {
              ...
              log.error(
                `[ipc/transcription] failed (preview-mix): model=${request.modelId} reason=${reason}`
              )
              if (!event.sender.isDestroyed()) {
                ...
                event.sender.send(channelId, {
                  event: 'failed',
                  error: isCancel ? 'Cancelled' : `Preview audio generation failed: ${reason}`
                })
              }
              return
            }
  ...
  ipcMain.handle(
    Channels.transcriptionDownloadModel,
    async (event, modelId: string): Promise<OkResult<{ channelId: string }> | ErrResult> => {
      try {
        assertValidModelId(modelId)
      } catch (err) {
  ...
  ipcMain.handle(
    `${Channels.transcriptionDownloadModel}:cancel`,
  ...
  ipcMain.handle(
    Channels.transcriptionListModels,
    async (): Promise<OkResult<ModelsState> | ErrResult> => {
      try {
  ...
  ipcMain.handle(
    Channels.transcriptionUninstallModel,
    async (_event, modelId: WhisperModelId): Promise<OkResult<ModelsState> | ErrResult> => {
      try {
        assertValidModelId(modelId)
        const modelsDir = getModelsDir()
        ...
        // REQ-0319 §1 — the auto-activate decision reads `activeModelId` and then
        // writes both it and `whisperModel`; all of that is one critical section.
        await mutateSettings((settings) => {
          if (settings.activeModelId === modelId) {
            ...
            settings.transcriptionDefaults = {
              ...settings.transcriptionDefaults,
              whisperModel: next ?? settings.transcriptionDefaults.whisperModel
            }
          }
  ...
  ipcMain.handle(
    Channels.transcriptionSetActiveModel,
    async (_event, modelId: WhisperModelId): Promise<OkResult<ModelsState> | ErrResult> => {
      try {
        assertValidModelId(modelId)
        const modelsDir = getModelsDir()
        ...
        if (!installed) {
          return {
            ok: false,
            error: { code: 'NOT_INSTALLED', message: `Model ${modelId} is not installed` }
          }
        }
